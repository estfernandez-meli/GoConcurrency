...
var orderFinishedMtx sync.Mutex
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup) {
	...
	for i := hunger; i > 0; i-- {
		//All but one will have pick up the left fork first (Plato)
		// Lock will block if the fork is in use, waiting until it's available again.
		...
		// Unlock releases the fork, making it available for other philosophers.
	...
	// Always lock and unlock when accessing a shared resource to ensure mutual exclusion.
	orderFinishedMtx.Lock()
	orderFinished = append(orderFinished, philosopher.name)
	orderFinishedMtx.Unlock()

	fmt.Println(philosopher.name, "is satisfied and left the table")
